        ...
        for(let i = 0; i < paginatedProducts.length; i++){
            disAccounts += `
                        <tr>
                            ...
                            <td>${paginatedProducts[i].cOrder}</td>
                        </tr>`
        }
...
function historyOrder(){
    ...
    for(let i = 0; i < deliveredOrder.length; i++){
        template += `
                    <li>Order #${i + 1} was marked as completed by Admin #1.</li>`
    }
...
function DlvSuccess(index){
    ...
    Accounts[comple].cOrder += 1;
    Accounts[comple].pendingOrder -= 1;
